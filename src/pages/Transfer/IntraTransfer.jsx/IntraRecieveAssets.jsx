...
const IntraRecieveAssets = () => {
  IntraRecieveAssets.propTypes = {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <Link to={`/intra_recieve_asset_preview/${row.original.assetId}`}>
            {/* <Button
              size="MD"
  ...
  const dataWithSlno = useMemo(() => {
    return responseData.map((item, index) => ({
      ...item,
      slno: index + 1,
      reqnumber: item.reqnumber.toUpperCase(), 
...
export default IntraRecieveAssets;
